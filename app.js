...
function renderSettings() {
  ...
  return heading('PRÉFÉRENCES', 'Paramètres', 'Gérez les notifications, vos données locales et les connexions futures.', '') +
    '<div class="two-columns"><div class="stack">' +
      ...
      card('PWA', '<div class="card-body"><p class="status ' + (window.matchMedia && window.matchMedia('(display-mode: standalone)').matches ? 'success' : 'info') + '">' + (window.matchMedia && window.matchMedia('(display-mode: standalone)').matches ? 'L’application est installée.' : 'L’application peut être installée depuis le menu du navigateur.') + '</p><p class="section-note" style="margin-top:12px">Les rappels lorsque l’application est fermée exigent un service de notification distant ou une application native. Le socle PWA et l’autorisation sont déjà en place.</p></div>') +
    '</div><div class="stack">' +
