...
async function discoverAddressBookHome() {
  ...
  const principalXml = parser.parse(await davFetch(root, { method: 'PROPFIND', headers: { Depth: '0' }, body: principalBody }));
  ...
  const homeBody = `<?xml version="1.0" encoding="UTF-8"?><d:propfind xmlns:d="DAV:" xmlns:card="urn:ietf:params:xml:ns:carddav"><d:prop><card:addressbook-home-set/></d:prop></d:propfind>`;
  const homeXml = parser.parse(await davFetch(principalUrl, { method: 'PROPFIND', headers: { Depth: '0' }, body: homeBody }));
...
async function listAddressBooks(homeUrl) {
  const body = `<?xml version="1.0" encoding="UTF-8"?><d:propfind xmlns:d="DAV:" xmlns:card="urn:ietf:params:xml:ns:carddav"><d:prop><d:resourcetype/><d:displayname/></d:prop></d:propfind>`;
  const xml = parser.parse(await davFetch(homeUrl, { method: 'PROPFIND', headers: { Depth: '1' }, body }));
  return asArray(xml.multistatus?.response).map(response => {
    const prop = propOf(response), type = prop.resourcetype || {};
    return { name: textValue(prop.displayname) || prop.displayname || 'Contacts', url: absolute(homeUrl, response.href), isAddressBook: Object.prototype.hasOwnProperty.call(type, 'addressbook') };
  }).filter(item => item.isAddressBook && item.url);
}

async function fetchVCards(book) {
  const body = `<?xml version="1.0" encoding="UTF-8"?><card:addressbook-query xmlns:d="DAV:" xmlns:card="urn:ietf:params:xml:ns:carddav"><d:prop><card:address-data/></d:prop><card:filter><card:prop-filter name="FN"/></card:filter></card:addressbook-query>`;
  ...
  for (const response of asArray(xml.multistatus?.response)) {
    const data = textValue(propOf(response)['address-data']);
    if (data && data.includes('BEGIN:VCARD')) cards.push(data);
  }
  return cards;
}
...
function birthdayEvents(card, startYear, endYear, bookName) {
  ...
  const name = field(lines, 'FN') || 'Contact', uid = field(lines, 'UID') || name, birthday = parseBirthday(field(lines, 'BDAY'));
  if (!birthday) return [];
  const events = [];
  for (let year = startYear; year <= endYear; year++) {
    if (!validDate(year, birthday.month, birthday.day)) continue;
    const mm = String(birthday.month).padStart(2, '0'), dd = String(birthday.day).padStart(2, '0');
...
module.exports = async function handler(req, res) {
  ...
  try {
    const now = new Date(), startYear = Number(req.query.startYear || now.getFullYear() - 1), endYear = Number(req.query.endYear || now.getFullYear() + 3);
    const books = await listAddressBooks(await discoverAddressBookHome());
    const details = [], events = [];
    for (const book of books) {
      try {
        const cards = await fetchVCards(book); let count = 0;
        for (const card of cards) { const generated = birthdayEvents(card, startYear, endYear, book.name); count += generated.length; events.push(...generated); }
